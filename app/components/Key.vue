<template>
  ...
  <div ref="keyButtonEl" @click="toggleModal" class="sm:hidden flex items-center justify-center w-11 h-9 p-1.5 relative cursor-pointer rounded-[4px]
  ...
  <div ref="keyEl" class="relative">
    <div v-if="modalOpen" class="absolute top-0.5 z-20 flex flex-col bg-transparent w-[268px]">
      ...
      <div
        ...
        <div class="flex items-center justify-between w-full p-2">
          <div class="flex flex-col justify-center h-full mb-px leading-3 text-xs overflow-hidden">
            <p class="flex font-mono tracking-tight text-[11px]/[11px] font-light text-slate-200 text-ellipsis">
              Recorded activities
            </p>
          </div>
        </div>
      ...
      <ul class="flex flex-wrap content-start p-0.5 bg-white rounded-b-[4px] drop-shadow-lg pb-1">
        <li v-for="(value, key) in keys" class="flex items-center w-32 h-7 m-0.5 overflow-hidden">
          <div :style="`background-color: ${value[1]}`" class="w-7 h-7 min-w-7 rounded-sm"></div>
          <div class="ml-1 flex flex-col justify-center h-full mb-px leading-3 text-xs overflow-hidden">
            <p class="ml-px text-slate-600 overflow-hidden text-ellipsis">
              {{ key }}
            </p>
            <div class="flex font-mono tracking-tight text-[11px]/[11px] font-light text-slate-400 text-ellipsis">
              [<span class="mr-0.5">{{ millisecondsToHoursAndMinutes(value[0]).hours }}h</span>{{
                millisecondsToHoursAndMinutes(value[0]).minutes }}m]
            </div>
          </div>
