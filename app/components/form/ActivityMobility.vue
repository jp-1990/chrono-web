<template>
  <side-panel id="activity-mobility" :is-open="!!props.mode" @on-close="onClose" @on-submit="onSubmit"
    @on-extra="onDelete">
    <template v-slot:title-text>{{ props.mode === 'update' ? "Update Mobility" : "Add Mobility" }}</template>
    ...
    <template v-slot:content>
      <div class="flex justify-between mb-1">
        ...
        <form-input-datetime v-model:value="formState.data.end" v-model:valid="formState.valid.end" label="End" required
...
const formState = ref<{ data: Activity<'mobility', 'form'>; valid: Validation }>({
  data: {
    id: '',
    title: 'mobility',
    variant: 'exercise',
