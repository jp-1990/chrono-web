<template>
  <side-panel :id="scope" :is-open="!!props.mode" @on-close="onClose" @on-submit="onSubmit" @on-extra="onDelete">
    <template v-slot:title-text>{{ props.mode === 'update' ? "Update Cardio" : "Add Cardio" }}</template>
    ...
    <template v-slot:content>
      <form-input-datetime ref="startRef" v-model:value="formState.data.start" v-model:valid="formState.valid.start"
        ...
        <div class="flex flex-col">
          ...
          <div class="flex">
            <span class="flex flex-col w-full">
              <form-input-text v-model:value="formState.data.data.exercise.title" v-model:valid="formState.valid.notes"
                label="Exercise" placeholder="Exercise" />
            </span>
            <span class="flex items-center w-[18px] ml-2 mr-1 font-bold text-xs" />
          </div>

          <div class="flex items-center ml-4 mt-2.5">
            <label :for="`${scope}-distance`" class="text-xs w-16">Distance</label>
            <input :id="`${scope}-distance`"
      ...
      <div class="flex mt-4 justify-between">
        ...
        <span class="ml-1 mr-7 text-xs font-light text-slate-500">[ Avg. pace: {{ avgPace }} ]</span>
      </div>

      <div v-for="(row, rowIndex) in formState.data.data.exercise.splits"
        :class="[rowIndex === 0 ? 'mt-1.5' : 'mt-2.5']" class="flex items-center ml-4 last-of-type:mb-1">

        <div class="flex items-center mr-4">
          <span for="update-exercise-reps" class="text-xs mr-1">Km<span class="ml-0.5 font-mono">{{ `${rowIndex < 10 ?
            `0${rowIndex + 1}` : rowIndex + 1}` }}</span></span>
            <label :for="`${scope}-pace-${rowIndex}`" class="text-xs ml-1 mr-1.5">Pace</label>
            <input :id="`${scope}-pace-${rowIndex}`"
              class="border w-10 h-6 py-1 px-1.5 rounded-[3px] focus:outline-none focus:border-slate-500 text-xs text-slate-700 placeholder:text-slate-400/70 placeholder:font-light"
              v-model="row.pace" />
        </div>

        <button v-if="rowIndex !== formState.data.data.exercise.splits.length - 1"
...
const removeSplitsRow = ({
  rowIndex
}: {
  rowIndex: number;
}) => {
  formState.value.data.data.exercise.splits = formState.value.data.data.exercise.splits.filter((_, i) => i !== rowIndex);
};
...
const formState = ref<{ data: Activity<'cardio', 'form'>; valid: Validation }>({
  data: {
    ...
    data: {
      exercise: { title: '', distance: '', duration: '', splits: [{ pace: '' }] }
    }
  },
  valid: {
...
function resetFormState() {
  ...
  formState.value.data.data.exercise = { title: '', distance: '', duration: '', splits: [{ pace: '' }] }
