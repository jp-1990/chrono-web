<template>
  <div ref="container" id="item-row" class="flex h-full w-full relative">
    <div v-if="ids" v-for="(id, index) in ids || []" v-on:mouseup="$emit('itemClick', $event, items?.[id])" :key="id"
      :style="`${items?.[id].style} background-color:${userState.activities[items?.[id].title ?? ''] ?? DEFAULT_COLOR}`"
      ...
      <div :id="`${items?.[id].id}-${index}-${id}-duration`" class="flex-1 bg-transparent h-full overflow-hidden"></div>
