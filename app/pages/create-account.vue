<template>
  <div class="h-screen flex items-center overflow-auto">
    <div class="flex flex-col flex-1 justify-center items-center">
      <div class="hidden sm:flex flex-col rounded-sm overflow-hidden drop-shadow-sm mb-12 mt-20">
      ...
      <section class="flex flex-col mb-12">
        <FormKit type="form" :actions="false" #default="{ disabled }" @submit="onSubmit" form-class="w-72"
          messages-class="flex justify-center">
          <FormKit type="text" name="givenName" id="givenName" label="First Name"
            label-class="text-slate-700 font-normal" />
          <FormKit type="text" name="familyName" id="familyName" label="Last Name"
            label-class="text-slate-700 font-normal" />
          <FormKit type="email" name="email" id="email" label="Email" placeholder="example@company.com"
            label-class="text-slate-700 font-normal" validation="email" />
          <FormKit type="password" name="password" id="password" label="Password" placeholder="********"
            label-class="text-slate-700 font-normal" validation="required|?length:8" />
          <FormKit type="password" name="confirmPassword" id="confirm-password" label="Confirm Password"
            placeholder="********" label-class="text-slate-700 font-normal" validation="required"
            validation-label="Password confirmation" />
          <div :class="[errors.length ? 'h-1' : 'h-8']"></div>
          <FormKit type="submit" :disabled="disabled" label="Create Account"
